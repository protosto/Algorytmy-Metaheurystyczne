...
public class EuclideanCoordinatesDistanceLoader implements DistanceLoader, DistanceMatrixCalculator {
    ...
    public double[][] calculateDistances(double[][] coordinates) {
        ...
        for(int i = 0; i < n; i++) {
            for(int j = i; j < n; j++) {
                distances[i][j] = Math.sqrt((coordinates[i][0] - coordinates[j][0]) * (coordinates[i][0] - coordinates[j][0]) + (coordinates[i][1] - coordinates[j][1]) * (coordinates[i][1] - coordinates[j][1]));
