...
public class AntColonySystemAlphaBetaTests {
    public static void runTests(List<String> fileNames) throws IOException {
        ...
        for (String fileName : fileNames) {
            bufferedWriterList.add(new BufferedWriter(new FileWriter(new File(path + fileName + "AlphaBeta.csv"))));
            distanceMatrices.add(TSPLoader.loadDistanceMatrixFromFile(fileName + ".tsp"));
        }

        for(double i = 0.0; i <= 2.01; i += 0.1) {
            for(double j = 0.0; j <= 2.01; j += 0.1) {
                AntColonySystem antColonySystem = new AntColonySystem(20, 0.1, 0.1, 1, 0.9, 100, i, j, 1, true, false, false);
